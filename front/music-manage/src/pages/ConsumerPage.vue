<template>
  <div class="table">
    ...
    <el-table size="mini" ref="multipleTable" border style="width:100%"
      ...
      <el-table-column label="会员身份" width="200" align="center">
        <template slot-scope="scope">
          <el-switch class="toggle-vip"
                     @change="toggleVip(scope.row)"
                     v-model="scope.row.isVipUser"
                     active-text="VIP"
                     inactive-text="非VIP">
          </el-switch>
        </template>
...
export default {
  ...
  methods: {
    ...
    toggleVip (user) {
      ...
      updVipStatus(res).then(resp => {
        if (resp.code === 200) {
          self.$message.success(resp.msg)
        }
      })
      console.log('>>>要提交的对象', res)
    },
    //获取当前页
